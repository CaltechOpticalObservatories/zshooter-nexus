Instrument Simulator
====================


.. rst-class:: zs-placeholder zs-internal-only

Words about the instrument simulator.


Simulator Example Notebooks
---------------------------

- :doc:`ZShooter Playground <_staged/sim/notebooks/playground_zshooter>`
- :doc:`PSF ADC <_staged/sim/notebooks/psf_adc>`


.. rst-class:: zs-placeholder zs-internal-only

Current plan for this page
--------------------------

- Add a link to a Binder/Google Colab of a Simulator environment
- Add simulator getting started docs

Useful external links
---------------------

.. container:: zs-note

   ScopeSim's own documentation is a great place to dig deeper. The instrument package discussed above allows
   the science team and potential users to explore the instrument's anticipated capabilities early.

- `ScopeSim documentation <https://scopesim.readthedocs.io/en/latest/>`_
- `ScopeSim GitHub repository <https://github.com/AstarVienna/ScopeSim>`_


.. toctree::
   :hidden:
   :maxdepth: 1
   :titlesonly:

   PSF ADC <_staged/sim/notebooks/psf_adc>
   ZShooter Playground <_staged/sim/notebooks/playground_zshooter>
